...
<head>
    ...
    <style>
        ...
        .nav-link .bi { margin-right: .5rem; color: #727272; }
        .nav-link.active { color: #2470dc; }
        .nav-link.active .bi { color: inherit; }
...
<header class="navbar navbar-dark sticky-top bg-dark flex-md-nowrap p-0 shadow">
  <a class="navbar-brand col-md-3 col-lg-2 me-0 px-3 fs-6" href="#">IT Ticketing Admin</a>
  ...
  <div class="navbar-nav">
    <div class="nav-item text-nowrap d-flex align-items-center px-3 text-white">
      <span class="me-3">Welcome, {{ auth()->user()->name }} ({{ auth()->user()->role }})</span>
      <form action="{{ route('logout') }}" method="POST">
          @csrf
          <button type="submit" class="nav-link bg-transparent border-0 text-white">Sign out</button>
      </form>
    </div>
  </div>
</header>
    ...
    <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block bg-light sidebar collapse">
      <div class="position-sticky sidebar-sticky">
        <ul class="nav flex-column">
          ...
          <li class="nav-item">
            <a class="nav-link {{ request()->is('admin/tickets') || request()->is('admin/tickets/*') ? 'active' : '' }}" href="{{ route('admin.tickets.index') }}">
              <i class="bi bi-ticket-detailed"></i> Tickets
            </a>
          </li>
          <li class="nav-item">
            <a class="nav-link {{ request()->is('admin/tickets-completed*') ? 'active' : '' }}" href="{{ route('admin.tickets.completed') }}">
              <i class="bi bi-archive"></i> Completed Tickets
            </a>
          </li>
          @if(auth()->user()->role === 'admin')
          <li class="nav-item">
            <a class="nav-link {{ request()->is('admin/users*') ? 'active' : '' }}" href="{{ route('admin.users.index') }}">
              <i class="bi bi-people"></i> Technician
            </a>
          </li>
          <li class="nav-item">
          ...
          <li class="nav-item mt-2">
            <a class="nav-link text-primary border-top pt-3" href="/">
              <i class="bi bi-arrow-left-circle"></i> View Public Site
            </a>
          </li>
        </ul>
      </div>
